...
int solution(vector<int> &A)
{
  ...
  long long lside = *(A.begin());
  long long rside = sum - lside;
  long long min_diff = abs(lside - rside);

  for (auto v = A.begin() + 1; v != A.end() - 1; v++)
  {
    lside += *v;
    rside -= *v;
    if (abs(rside - lside) <= min_diff)
    {
      min_diff = abs(rside - lside);
    }
  }
  return min_diff;
}
